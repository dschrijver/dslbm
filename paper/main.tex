...
\begin{split}
    ...
    B_{\alpha\beta} &\equiv \Pi_{\alpha\beta}^{\text{NS}} - A_{\alpha\beta},\\
    A_{\alpha\beta} &\equiv 2\sum^\text{out}_i c_{i\alpha}c_{i\beta}f_i + \sum^\text{par}_i c_{i\alpha}c_{i\beta}f_i \\
    &\quad + \sum_i^\text{in}c_{i\alpha}c_{i\beta}\left(2w_i \rho \frac{\bm{u} \cdot \bm{c}_i}{c_s^2} - \sum_\gamma w_i c_{i\gamma}N_\gamma\right).
\end{split}
\end{equation}
For a boundary in the $yz$, $xz$ and $xy$-planes, Eq.~(\ref{eq:matrix equation}) can be solved if we use the D3Q27 stencil,

\onecolumngrid
\begin{equation}
    \underline{\underline{M}} = \begin{pmatrix}
    -12 B_{00} + 9 B_{11} + 9 B_{22} & -18 B_{01} & -18 B_{02}\\
    -18 B_{01} & 9 B_{00} - 27 B_{11} & -54 B_{12}\\
    -18 B_{02} & -54 B_{12} & 9 B_{00} - 27 B_{22}
    \end{pmatrix}
\end{equation}

\begin{equation}
    \underline{\underline{M}} = \begin{pmatrix}
    -27 B_{00} + 9 B_{11} & -18 B_{01} & -54 B_{02}\\
    -18 B_{01} & 9 B_{00} - 12 B_{11} + 9 B_{22} & -18 B_{12}\\
    -54 B_{02} & -18 B_{12} & 9 B_{11} - 27 B_{22}
    \end{pmatrix}
\end{equation}

\begin{equation}
    \underline{\underline{M}} = \begin{pmatrix}
    -27 B_{00} + 9 B_{22} & -54 B_{01} & -18 B_{02} \\
    -54 B_{01} & -27 B_{11} + 9 B_{22} & -18 B_{12} \\
    -18 B_{02} & -18 B_{12} & 9 B_{00} + 9 B_{11} - 12 B_{22}
    \end{pmatrix}
\end{equation}
\twocolumngrid
