...
\begin{split}
    ...
    &= \frac{1}{A_\alpha}\left[\sum_i^\text{par}c_{i\alpha}\left(f_i^\sigma - f_i^{\text{eq},\sigma}\right)+ \frac{1}{2}F_\alpha^\sigma\right],
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
    f_0^\sigma &= f_0^{*,\sigma} + \Delta m^\sigma\\
...
\begin{split}
    ...
    &f_i^{(3)} = f_i^{\text{eq},(3)} -\frac{\tau}{\Delta t}\Biggl(\Delta x c_{i\alpha}\partial_\alpha f_i^{(2)} + \frac{1}{2}\Delta x^2 c_{i\alpha}c_{i\beta}\partial_\alpha\partial_\beta f_i^{(1)}+ \Delta t \partial_t f_i^{(1)} + \Delta x  c_{i\alpha}\partial_\alpha \Delta t\partial_t f_i^{(0)}\\
...
\begin{split}
    ...
    N_\alpha &= \frac{1}{A_\alpha}\left[\sum_i^\text{par}c_{i\alpha}\left(f_i - f_i^{\text{eq}}\right)+ \frac{\Delta x}{2}F_\alpha\right].
\end{split}
\end{equation}
Note that we ignored the component specifier $\sigma$ for simplicity. We can expand this equation in Knudsen number. The momentum corrections are given as follows,
\begin{equation}
\begin{split}
    N_\alpha^{(0)} &= \frac{1}{A_\alpha}\sum_i^\text{par}c_{i\alpha}\left(f_i^{(0)} - f_i^{\text{eq},(0)}\right),\\
    N_\alpha^{(1)} &= \frac{1}{A_\alpha}\left[\sum_i^\text{par}c_{i\alpha}\left(f_i^{(1)} - f_i^{\text{eq},(1)}\right)+ \frac{\Delta x}{2}F_\alpha^{(0)}\right],\\
    N_\alpha^{(2)} &= \frac{1}{A_\alpha}\left[\sum_i^\text{par}c_{i\alpha}\left(f_i^{(2)} - f_i^{\text{eq},(2)}\right)+ \frac{\Delta x}{2}F_\alpha^{(1)}\right].
\end{split}
\end{equation}
As can be seen from Eq.~(\ref{eq:asymptotic expansion lbe}), the zeroth-order distribution function is equal to the zeroth-order equilibrium. This means that the momentum correction at this order is zero, and the NEBB is satisfied. The first-order distribution function gives the following momentum corrections,
\begin{equation}
...
\end{equation}
For the equilibrium and source term defined by previous research~\cite{saito_generalized_2023}, we get the following if we assume $\bm{u}^{(0)} = \bm{0}$,
\begin{equation}
\begin{split}
    \sum_i^\text{par}c_{i\alpha}c_{i\beta}f_i^{\text{eq},(0)} &= (\delta_{\alpha n} - 1)\delta_{\alpha\beta}\frac{2}{3}p^{(0)},\\
    \sum_i^\text{par}c_{i\alpha}S_i^{(0)} &= (\delta_{\alpha n} - 1)\frac{2}{3}F_\alpha^{(0)},
\end{split}
\end{equation}
where $p$ is the pressure. This gives the following momentum correction,
\begin{equation}
\begin{split}
    w_i c_{i\alpha}N_\alpha^{(1)} &= \frac{w_i c_{i\alpha}}{\sum_i^\text{in}w_i c_{i\alpha}^2}\left[ - \frac{2}{3}\frac{\tau}{\Delta t}(\delta_{\alpha n} - 1)\left(\partial_\alpha p^{(0)} - \Delta x F_\alpha^{(0)}\right)+ \frac{\Delta x}{2}F_\alpha^{(0)}\right]\\
    &= \sum_\alpha \left[-12\frac{\tau}{\Delta t}(\delta_{\alpha n} - 1)w_i c_{i\alpha}\left(\partial_\alpha p^{(0)}- \Delta xF_\alpha^{(0)}\right) + 3 w_i c_{i\alpha}\Delta xF_\alpha^{(0)}\right]
\end{split}
\end{equation}
